...
static u32 entry_to_index(struct zram *zram, struct zram_table_entry_list *entry)
{
	if ((unsigned long)entry < (unsigned long)zram->table)
		return zram->disksize >> PAGE_SHIFT;
	return (u32)(((unsigned long)entry - (unsigned long)zram->list_table) /
			sizeof(struct zram_table_entry_list));
}
...
static void zram_writeback_clear_flag(struct zram *zram, u32 index)
{
	...
	if (zram_allocated(zram, index)) {
		...
		if (!list_empty(&zram->list_table[index].lru_list))
			list_move_tail(&zram->list_table[index].lru_list, &zram->list);
		else
			list_add_tail(&zram->list_table[index].lru_list, &zram->list);
...
static int zram_wbd(void *p)
{
	...
	struct zram_table_entry_list *zram_entry, *n;
	struct zram_writeback_buffer *buf = NULL;
	u32 index;
	int ret;
		...
		list_for_each_entry_safe(zram_entry, n, &zram->list, lru_list) {
			...
			if (!ret) {
				...
				n = list_first_entry(&zram->list,
						struct zram_table_entry_list, lru_list);
			}
...
void zram_add_to_writeback_list(struct list_head *list, unsigned long index)
{
	...
	if (zram_allocated(zram, index) &&
		...
		spin_lock_irqsave(&zram->list_lock, flags);
		if (!list_empty(&zram->list_table[index].lru_list)) {
			list_move(&zram->list_table[index].lru_list, list);
...
{
	struct zram *zram = g_zram;
	struct zram_table_entry_list *entry;
	u32 index;
	unsigned long flags;
	...
	while (!list_empty(list)) {
		...
		spin_lock_irqsave(&zram->list_lock, flags);
		if (!list_empty(&zram->list_table[index].lru_list))
			list_del_init(&zram->list_table[index].lru_list);
		spin_unlock_irqrestore(&zram->list_lock, flags);
		zram_slot_unlock(zram, index);
	}
	return 0;
...
static bool zram_meta_alloc(struct zram *zram, u64 disksize)
{
	...
	if (zram->backing_dev) {
		...
		zram->list_table = vzalloc(array_size(num_pages, sizeof(*zram->list_table)));
		if (!zram->list_table) {
			zs_destroy_pool(zram->mem_pool);
			vfree(zram->table);
			return false;
		...
		for (i = 0; i < num_pages; i++)
			INIT_LIST_HEAD(&zram->list_table[i].lru_list);
	}
#endif
...
out:
	...
	if (zram_test_flag(zram, index, ZRAM_UNDER_PPR))
		zram_clear_flag(zram, index, ZRAM_UNDER_PPR);
	spin_lock_irqsave(&zram->list_lock, flags);
	if (zram->list_table && !list_empty(&zram->list_table[index].lru_list)) {
		list_del_init(&zram->list_table[index].lru_list);
		if (zram_test_flag(zram, index, ZRAM_LRU)) {
			zram_clear_flag(zram, index, ZRAM_LRU);
			atomic64_dec(&zram->stats.lru_pages);
...
#ifdef CONFIG_ZRAM_LRU_WRITEBACK
		if (zram->list_table && (!page->mem_cgroup ||
